...
$\egame(\mu p.\phi,\bbS)@(\mu p.\phi,s)$;% such that each point in $T_{\chi,s}$ lies in some path belonging to $B^\model(\chi')$,
...
$\UG_F@s$.% such that each point in $T_{\chi,s}$ lies in some path belonging to $B^\model(\chi')$,
\end{enumerate}
\end{proposition}
\begin{proof}
