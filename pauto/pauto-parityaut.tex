...
Recall that we write $\llang^+(A)$ to denote the fragment of $\llang(A)$ where every predicate $a\in A$ occurs only positively.
...
\end{center}
%
$\aut$ \emph{accepts} $\model$ if $\eloise$ has
a winning strategy in $\agame(\aut,\model)@(a_I,s_I)$, and \emph{rejects} $\model$
...
if for every structure $(D,\val)$ we have $(D,\val) \models \varphi \quad\text{iff}\quad (D,\val^{c}) \not\models \psi$, where $\val^{c}$ is the valuation given by $\val^{c}(a) \mathrel{:=} D
...
For each $\aut \in \Aut(\llang)$ and $\model$ we have that $\dual{\aut}$ accepts $\model$
if and only if $\aut$ rejects $\model$.
\end{proposition}
...
Any weak parity automaton $\bbA$ is equivalent to one $\bbA'$ with $\pmap: A' \to \{0,1\}$. This is because \emph{(weakness)} prevents states of different parity to occur infinitely often in acceptance games; so we may just assign $0$ to any even-parity and $1$ to any odd-parity state. We shall assume such a restricted parity map for weak parity automata.
\end{remark}



The next two sections will use parity automata to characterise $\wmso$ and $\nmso$. In Section \ref{sec:autwmso} we will show that $\wmso$ equivalent to $\AutWC(\olque)$ on trees. in Section \ref{sec:autnmso}, we will relate in the same way $\nmso$ and $\AutW(\ofoe)$.
