...
%%
%%
%A seminal result in the theory of modal logic is van Benthem's Bisimulation
%Theorem~\cite{vanBenthemPhD}, stating that every first-order formula
%$\alpha(x)$ which is invariant under bisimulations, is actually equivalent to
%(the standard translation of) a modal formula.
%Concisely, this result can be formulated as follows:
%\begin{equation}
%\label{eq-vB}
%\m{FO}/{\bis} = \ML.
%\end{equation}
%Over the years, a wealth of variants of the Bisimulation Theorem have been
%obtained.
...
%
%Of particular interest to us is the work of Janin \& Walukiewicz~\cite{Jan96},
%who extended van Benthem's result to the setting of fixpoint logics, by
%proving that the modal $\mu$-calculus ($\MC$) is the bisimulation-invariant
...
%\end{equation}
%
%The general pattern of these results takes the following shape:
%\begin{equation}
%\label{eq-pattern}
%\mc{L}/{\bis} = \mc{M} \hspace*{5mm}(\text{over } \mc{C}),
%\end{equation}
%stating that $\mc{M}$ is the bisimulation-invariant fragment of $\mc{L}$
%over a class $\mc{C}$ of models.
%Here $\mc{L}$ is some yardstick logic such as first-order logic ($\m{FO}$),
%monadic second-order logic ($\MSO$) or weak monadic second-order logic
%($\wmso$); $\mc{M}$ is some variant of modal logic such as the modal
%$\mu$-calculus or one of its most important fragments: the
%alternation-free fragment ($\afmc$); and $\mc{C}$ is some class of
%models, such as finite, transitive, or tree models.
%Table~\ref{fig:table} summarizes some important results following this
%pattern.
%
%\begin{table}
%\centering
...
%\hline
%Structures ($\mc{C}$)& $\mc{L}$ & $\mc{M}$  &  reference
%\\ \hline \hline
%\multirow{4}{*}{TSs}
% &  FO  & ML  & \cite{vanBenthemPhD} \\
% & MSO  & MC & \cite{Jan96} \\
% & WMSO & ?  & -- \\
% &  ?   & afmc & --
%\\ \hline
%\multirow{1}{*}{binary trees}
% & WMSO & afmc & \cite{ArnNiw92}
%\\ \hline
%\multirow{2}{*}{finite TSs}
% & FO         & ML & \cite{rose:moda97} \\
% & WMSO = MSO & ?  & --
%\\ \hline
%  \multirow{2}{*}{transitive TSs} %& MSO & $\mu$-calculus \\
% & WMSO & ML & \cite{CateF11} \\
% &  MSO & afmc & \cite{AlberucciF09,DawarO09}
%\\ \hline
%\end{tabular}
%\caption{\rm TS stands for `transition system'.}
%\label{fig:table}
%\end{table}
%
%Table~\ref{fig:table} suggestively indicates the existence of some open
%problems, but let us first address the issue \emph{why} characterization
%results of the form \eqref{eq-pattern} are of interest, apart from their
%obvious importance in (finite) model theory.
%The point is that the mentioned logics, and the models they are interpreted
%in, feature prominently in the area of formal verification theory.
%Generally, one is interested in applications where these models are
%transition structures representing certain computational processes, and one
%usually takes the point of view that bisimilar models represent the
%\emph{same} process.
%For this reason, properties of transition structures that are not
%bisimulation-invariant are simply irrelevant.
%Seen in this light, \eqref{eq-pattern} is an \emph{expressive
%completeness} result, stating that all relevant properties of $\mc{L}$
%(which is generally some kind of expressive yardstick formalism), can
%already be expressed in a (usually computationally more feasible)
%fragment $\mc{M}$.
%Or, conversely starting from $\mc{M}$, according to \eqref{eq-pattern},
%one may think of $\mc{L}$ as an extension of $\mc{M}$ that is completely
%covered by $\mc{M}$ when it comes to expressing relevant properties.
%
%In this paper, which is based on an MSc thesis \cite{Zanasi:Thesis:2012} written by the third
%author and supervised by the first two authors, we try to improve our
%grasp of such `expressiveness modulo bisimilarity' results.
%We are particularly interested in the relation between (variants of) monadic
%second-order logic and modal fixpoint logics; that is, in variants of the
%Janin-Walukiewicz result \eqref{eq-JW}.
%More concretely, we fill in one of the three gaps of Table~\ref{fig:table}
%by providing a natural solution $\mc{L}$ to the equation
%\begin{equation}
%\label{eq-MR1}
%\mc{L}/{\bis} = \afmc \hspace*{5mm}(\text{over all TSs}).
%\end{equation}
%
%
%Naively, one might think that when considering this question in the context
%of all transition systems, the situation is the same as for the class of
%binary trees~\cite{ArnNiw92}, so that $\mc{L} = \wmso$ would solve
%\eqref{eq-MR1}.
%But if this were the case, then $\afmc$ would also be the
%bisimulation-invariant fragment of $\wmso$ over trees.
%However, it turns out that the class of well-founded trees, which is
%definable in $\afmc$ by the formula $\mu x. \square x$, is not
%$\wmso$-definable.
%The reason comes from topology: the class of well-founded trees is not Borel,
%whereas all $\wmso$-definable tree languages are Borel.
%On the other hand, it is not clear either whether the bisimulation-invariant
%fragment of $\wmso$ is included in $\afmc$ (or even in the modal
%$\mu$-calculus itself), no matter how reasonable this may seem.
%The point is that, contrary to the finitely branching case, $\wmso$ is
%not a fragment of $\MSO$ over trees of arbitrary branching degree.
%(In fact the two logics are incomparable, as a consequence of the following
%\emph{finite branching property} of $\MSO$: every non-empty $\MSO$
%definable tree language contains at least a finitely branching
%tree \cite{Walukiewicz96}.
%It follows that $\MSO$ cannot define the class of infinitely branching
%trees, which on the contrary is clearly $\wmso$-definable.)
%
%It turns out that in order to solve the equation \eqref{eq-MR1}, we need to
%introduce a \emph{new} variant of monadic second-order logic.
%In this variant, that we shall call \emph{well-founded} monadic
%second-order logic ($\WFMSO$), the second-order quantifiers range over
%special subsets of the transitions system that we call \emph{noetherian}.
%Roughly, a subset $S$ of a transition system $\mb{T}$ is noetherian if there
%are no infinite paths in the past of $S$ (a more precise definition follows).
%Note that a subset $S$ in a tree model $\mb{T}$ is noetherian iff $S$ is a
%subset of a (conversely) well-founded subtree of $\mb{T}$ --- this explains
%our terminology.
...
tree.\footnote{As remarked in \cite{CateF11}, this follows from the automata characterization of MSO given in \cite{Walukiewicz96}.} %\afnote{This follows implicitly from Igor's automata characterization of MSO, cf. props. 2 in my paper with Balder.}
